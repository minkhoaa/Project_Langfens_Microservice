...
public class AdminExamService_UnitTests_Validation
{
    ...
    [Fact]
    public async Task AddAsync_Should_Generate_Slug_From_Title_And_Make_Unique()
    {
        ...
        var svc = new AdminExamService(ctx);
        var dto = new DtoAdmin.AdminExamCreate("IELTS Đọc 1", null, null, ExamCategory.IELTS!, ExamLevel.B1!, 60);
        var (status, api) = ResultHelpers.Extract<ApiResultDto>(await svc.AddAsync(dto, CancellationToken.None));

        Assert.Equal(StatusCodes.Status200OK, status);
        Assert.True(api!.isSuccess);
        var created = await ctx.Exams.OrderByDescending(e => e.CreatedAt).FirstAsync();
        Assert.Equal("ielts-doc-1-2", created.Slug);
    }
}
