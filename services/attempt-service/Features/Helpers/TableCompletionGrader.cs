...
/// <summary>
/// Grader for <c>TABLE_COMPLETION</c>. Per-gap normalised exact match
/// (delegates to <see cref="CompletionScoring"/>).
/// </summary>
public sealed class TableCompletionGrader(ITextNormalizer normalizer) : IGrader
{
    public string Slug => "TABLE_COMPLETION";

    private static readonly JsonSerializerOptions Opts = new(JsonSerializerDefaults.Web);

    public GradeResult Grade(AnswerEnvelope learner, AnswerEnvelope correct, NormalizationConfig norm)
    {
        var correctAns = (TableCompletionAnswer)correct.TypedRecord;
