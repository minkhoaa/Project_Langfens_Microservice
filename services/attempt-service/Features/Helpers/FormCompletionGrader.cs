...
/// <summary>
/// Grader for <c>FORM_COMPLETION</c>. Per-gap normalised exact match
/// (delegates to <see cref="CompletionScoring"/>).
/// </summary>
public sealed class FormCompletionGrader(ITextNormalizer normalizer) : IGrader
{
    public string Slug => "FORM_COMPLETION";

    private static readonly JsonSerializerOptions Opts = new(JsonSerializerDefaults.Web);

    public GradeResult Grade(AnswerEnvelope learner, AnswerEnvelope correct, NormalizationConfig norm)
    {
        var correctAns = (FormCompletionAnswer)correct.TypedRecord;
