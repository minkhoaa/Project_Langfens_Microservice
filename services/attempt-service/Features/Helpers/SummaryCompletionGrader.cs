...
/// <summary>
/// Grader for <c>SUMMARY_COMPLETION</c>. Per-gap normalised exact match
/// (delegates to <see cref="CompletionScoring"/>).
/// </summary>
public sealed class SummaryCompletionGrader(ITextNormalizer normalizer) : IGrader
{
    public string Slug => "SUMMARY_COMPLETION";

    private static readonly JsonSerializerOptions Opts = new(JsonSerializerDefaults.Web);

    public GradeResult Grade(AnswerEnvelope learner, AnswerEnvelope correct, NormalizationConfig norm)
    {
        var correctAns = (SummaryCompletionAnswer)correct.TypedRecord;
