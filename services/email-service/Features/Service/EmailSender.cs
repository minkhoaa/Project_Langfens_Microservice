...
namespace email_service.Features.Service
{
    ...
    public class EmailSender : IEmailSender
    {
        ...
        public async Task SendAsync(string toEmail, string subject, string html, string? plain = null, CancellationToken token = default)
        {
            var msg = new MimeMessage();
            msg.From.Add(MailboxAddress.Parse(smtpOptions.From));
            ...
            var secure = smtpOptions.Port == 465 ? SecureSocketOptions.SslOnConnect : SecureSocketOptions.StartTls;
            await client.ConnectAsync(smtpOptions.Host, smtpOptions.Port, secure, token);
            if (!string.IsNullOrEmpty(smtpOptions.User))
                await client.AuthenticateAsync(smtpOptions.User, smtpOptions.Pass, token);
        ...
        {
            var html = BuildOtpHtml(toEmail, otp, ttlSeconds, smtpOptions);
            var plain = BuildOtpPlain(toEmail, otp, ttlSeconds, smtpOptions);
